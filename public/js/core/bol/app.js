...
function NXBootCategories() {
    ...
    //[bol]
    if (page_section == 'bol') {
        if ($("#settings-menu-bols").attr('aria-expanded') == 'false') {
            $("#settings-menu-bols").trigger('click');
            $("#settings-menu-categories-bol").addClass('active');
        }
    }
...
function NXPostGeneralComment() {
    ...
    tinymce.init({
        ...
        height: 200,
        toolbar: 'formatselect undo redo bold | image link | alignleft aligncenter alignright outdent indent bullist numlist | hr table blockbol code',
        //autosave/update text area
        setup: function (editor) {
            editor.on('change', function () {
                editor.save();
...
/**--------------------------------------------------------------------------------------
 * [BOL - CLONE] 
 * @blade : bols\components\modals\clone.blade.php
 * @description: validation for cloning an bol
 * -------------------------------------------------------------------------------------*/
function NXBolClone() {
    //validation
    $("#commonModalForm").validate().destroy();
...
/**--------------------------------------------------------------------------------------
 * [BOL - RECURRING] 
 * @blade : bols\components\modals\recurring-settings.blade.php
 * @description: validation for recurring an bol
 * -------------------------------------------------------------------------------------*/
function NXBolRecurring() {
    //validation
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        rules: {
...
/**--------------------------------------------------------------------------------------
 * [BOL - ADD] 
 * @blade : bols\components\modals\add-edit-inc.blade.php
 * @description: validation for creating an bol
 * -------------------------------------------------------------------------------------*/
function NXBolCreate() {
    //validation
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        rules: {
...
/**--------------------------------------------------------------------------------------
 * [STRIPE BUTTON] 
 * @description: geneate button shown on bol page
 * -------------------------------------------------------------------------------------*/
function NXStripePaymentButton() {
    ...
    $(document).on('click', '#bol-stripe-payment-button', function () {
...
function NXRazorpayPaymentButton() {
    ...
    $(document).ready(function () {
        $('body').on('click', '#bol-razorpay-payment-button', function (e) {
            //get vars
 ...
 * @description: form validation
 * -------------------------------------------------------------------------------------*/
function NXSettingsBols() {
    $("#settingsFormBols").validate({
        rules: {
            settings_bols_recurring_grace_period: "required",
        },
        submitHandler: function (form) {
            nxAjaxUxRequest($("#commonModalSubmitButton"));
        }
...
/**--------------------------------------------------------------------------------------
 * Form validation for converting an estimate to an bol
 * @description: form validation
 * -------------------------------------------------------------------------------------*/
function NXConvertEstimetToBol() {
    //create status - form validation
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        rules: {
