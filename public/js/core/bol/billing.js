 ...
 * [returns] - bool
 *--------------------------------------------------------------*/
NXBOL.log = function (payload1, payload2) {
 ...
 *--------------------------------------------------------------*/
NXBOL.toggleTaxDiscountButtons = function (value = 0) {

    var bill_final_amount = Number($("#bill_final_amount").val());
    var bill_subtotal = Number($("#bill_subtotal").val());

    //log
    NXBOL.log('[billing] toggleTaxDiscountButtons() - based on billing value [bill_final_amount]: (' + bill_final_amount + ')');
...
NXBOL.DOM.domState = function () {

    NXBOL.log('[billing] state() - setting billing doma state- [payload]', NXBOL.DATA.BOL);

    //toggle discount and tax buttons

    NXBOL.toggleTaxDiscountButtons();
 ...
 *--------------------------------------------------------------*/
NXBOL.toggleTaxDom = function (tax_type = '') {



    NXBOL.log('[billing] initialiseTaxPopover() - toggling tax popover - [tax_type]: ' + tax_type);
 ...
 *--------------------------------------------------------------*/
NXBOL.toggleDiscountDom = function (bill_discount_type = '') {

    //get current values
    var bill_discount_value = '';

    //log
    NXBOL.log('[billing] toggleDiscountDom() setting popover state [current_discount_type]: (' + bill_discount_type + ')');
 ...
 *--------------------------------------------------------------*/
NXBOL.toggleAdjustmentDom = function () {

    NXBOL.log('[billing] toggleAdjustmentDom() setting adjustment popover state');
...
NXBOL.DOM.updateAmountDue = function (bill_final_amount = 0) {

    NXBOL.log("[billing] NXBOL.DOM.updateAmountDue() - updating amount due label [bill_final_amount]: (" + bill_final_amount + ")");
 ...
 *--------------------------------------------------------------*/
NXBOL.DOM.itemNewLine = function (data = {}) {

    NXBOL.log("[billing] NXBOL.DOM.itemNewLine() - cloning time line - [payload]");
    NXBOL.log(data);
    ...
    if (item_linked_type != '') {
        ...
        if ($("input[data-duplicate-check='" + check + "']").length > 0) {
            NXBOL.log("[billing] NXBOL.DOM.itemNewLine() - the item being added is a duplicate. Will skip (" + check + ")");
            //note this duplcate error
            NXBOL.DATA.expense_duplicate_count++;
            return;
        }
    }
 ...
 * -add new blank line item
 *--------------------------------------------------------------*/
NXBOL.DOM.timeNewLine = function (data = {}) {

    NXBOL.log("[billing] NXBOL.DOM.timeNewLine() - cloning time line - [payload]");
    NXBOL.log(data);

    //get data is any was provided
    var item_description = (data.item_description != null) ? data.item_description : '';
    var item_unit = (data.item_unit != null) ? data.item_unit : NXLANG.bol_time_unit;
 ...
 * ------------------------------------------------------------------------------------------------------------------*/
NXBOL.CALC.recalculateLines = function () {

    NXBOL.log("[billing] recalculateLines() - validating and recalculating each line item");

    //(1) ------------------------------ find each line item --------------------------------
    $("#billing-items-container").find(".billing-line-item").each(function () {

        NXBOL.log("[billing] recalculateLines() - found a line item. Now validating and calculating it");
        ...
        NXBOL.log("[billing] recalculateLines() - [quantity]: (" + quantity + ") - [rate]: (" + rate + ") - [linetax]: (" + line_tax + ") - [type]: (" + type + ")");
        ...
        if (type == 'plain') {
            ...
            if (quantity > 0 && rate > 0) {
                ...
                tax.val(linetax);
                //increase bill total
                NXBOL.DATA.calc_total += linetotal;
                NXBOL.log("[billing] reclaculateBill() - line item is valid. [line item total]: " + linetotal);
            } else {
                NXBOL.log("[billing] reclaculateBill() - line item is invalid and is skipped");
                total.val('');
            }
        }
 ...
 * -delete line item
 *--------------------------------------------------------------*/
NXBOL.DOM.deleteLine = function (self) {

    NXBOL.log("[billing] NXBOL.DOM.deleteLine() - deleteing line item");
 ...
 *--------------------------------------------------------------*/
NXBOL.DOM.addSelectedProductItems = function (self) {

    NXBOL.log("[billing] NXBOL.DOM.addSelectedProductItems() - adding items selected in add items modal");
 ...
 *--------------------------------------------------------------*/
NXBOL.DOM.addSelectedExpense = function (self) {

    NXBOL.log("[billing] NXBOL.DOM.addSelectedExpense() - adding expenses selected in add expenses modal");

    //count
    var count_selected = 0;

    //duplicates checker
    NXBOL.DATA.expense_duplicate_count = 0;
    ...
    if (NXBOL.DATA.expense_duplicate_count) {
        NX.notification({
            type: 'error',
            message: NXLANG.selected_expense_is_already_on_bol
        });
    }
 ...
 *--------------------------------------------------------------*/
NXBOL.DOM.addSelectedTimebilling = function (self) {

    NXBOL.log("[billing] NXBOL.DOM.addSelectedTimebilling() - adding hours selected in add time billing modal");
 ...
 *--------------------------------------------------------------*/
NXBOL.updateTax = function () {

    //get tax type from popover form
    var tax_type = $("#billing-tax-type").val();


    NXBOL.log("[billing] updateTaxType() - updating tax type [type]: (" + tax_type + ")");
    ...
    //recalculate bill
    NXBOL.CALC.reclaculateBill(self);
}
 ...
 * -
 *--------------------------------------------------------------*/
NXBOL.updateAdjustment = function () {

    NXBOL.log("[billing] updateAdjustment() - updating bill adjustment amount");
 ...
 * -
 *--------------------------------------------------------------*/
NXBOL.removeAdjustment = function () {

    //log
    NXBOL.log("[billing] removeAdjustment() - updating bill adjustment amount to zero");
 ...
 *--------------------------------------------------------------*/
NXBOL.updateDiscount = function () {

    NXBOL.log("[billing] updateDiscountType() - updating discount type");
    ...
    if (discount_type == 'percentage') {
        NXBOL.log("[billing] updateDiscountType() - [percentage] ", Number($("#js_bill_discount_percentage").val()));
    ...
    if (discount_type == 'fixed') {
        NXBOL.log("[billing] updateDiscountType() - [fixed] ", Number($("#js_bill_discount_amount").val()));
 ...
 *--------------------------------------------------------------*/
NXBOL.CALC.reclaculateBill = function (self) {

    NXBOL.log("[billing] reclaculateBill() - recalculating bill");
    ...
    //recalculate lines
    NXBOL.CALC.recalculateLines();



    //(1) ----------------------- SUM UP LINE ITEMS -------------------------------------
    NXBOL.log("[billing] reclaculateBill() - summing up all line items - started");
    $("#billing-items-container").find(".billing-line-item").each(function () {
        //each line
        var lineitem = $(this);
        ...
        if (typeof linetotal == 'number') {
            NXBOL.log("[billing] reclaculateBill() - valid line item found [total]: (" + linetotal + ")");
            bill_subtotal += linetotal;
        }
    });

    //(2) ----------------- UPDATE SUBTOTAL (SUM BEFORE ADJUSTMENTS)--------------------------------
    NXBOL.log("[billing] reclaculateBill() - updating subtotal [subtotal]: (" + bill_subtotal + ") ");
    ...
    NXBOL.log("[billing] reclaculateBill() - calculating discounts [bill_discount_type]: (" + bill_discount_type + ") [bill_discount_percentage]: (" + bill_discount_percentage + ") [bill_discount_amount] (" + bill_discount_amount + ")");
    ...
    if (bill_discount_type == 'percentage') {
        ...
        //log
        NXBOL.log("[billing] reclaculateBill() - discount is percentage based. [bill_discount_amount]: (" + bill_discount_amount + ")");
    }
    ...
    if (bill_discount_type == 'fixed') {
        //log
        NXBOL.log("[billing] reclaculateBill() - discount is fixed. [bill_discount_amount]: (" + bill_discount_amount + ")");
    }
    ...
    if (bill_discount_amount > 0) {
        ...
        //log
        NXBOL.log("[billing] reclaculateBill() - there is a discount - setting DOM  and [bill_amount_before_tax]: (" + bill_amount_before_tax + ")");
        ...
        //log
        NXBOL.log("[billing] reclaculateBill() - there is no discount - [bill_amount_before_tax]: (" + bill_amount_before_tax + ")");
    }
    ...
    if ($("#bill_tax_type").val() == 'summary') {
        //log
        NXBOL.log("[billing] reclaculateBill() - calculating summary taxes");
        ...
        if (bill_tax_total_percentage > 0) {
            //log
            NXBOL.log("[billing] reclaculateBill() - this bill has [summary] based tax [tax_percentage]: (" + bill_tax_total_percentage + "%)");
            //tax calculation
            ...
            //log
            NXBOL.log("[billing] reclaculateBill() - the total tax on this bill is [total_tax_amount]: (" + bill_tax_total_amount + ")");
        } else {
            NXBOL.log("[billing] reclaculateBill() - this bill does not have any applicable tax");
    ...
    NXBOL.log("[billing] reclaculateBill() - total amount before adjustment. [bill_final_amount]: (" + bill_final_amount + ")");

    //adjustment
    NXBOL.log("[billing] reclaculateBill() - accounting for the adjustment. [bill_adjustment_amount]: (" + bill_adjustment_amount + ")");
    ...
    NXBOL.DOM.updateAmountDue(bill_final_amount);

    NXBOL.log("[billing] reclaculateBill() - bill claculation finished [final amount]: (" + bill_final_amount + ")");
 ...
 * [SAVE BOL BUTTON]
 *--------------------------------------------------------------*/
NXBOL.CALC.saveBill = function (self) {

    NXBOL.log("[billing] saveBill() - started");

    if (NXBOL.CALC.validateLines()) {
 ...
 * [SAVE BOL BUTTON]
 *--------------------------------------------------------------*/
NXBOL.CALC.validateLines = function (self) {

    //log
    NXBOL.log("[billing] validateLines() - started");
    ...
    //log
    NXBOL.log("[billing] validateLines() - (" + count_bill_error + ") error found");
 ...
 *--------------------------------------------------------------*/
NXBOL.DOM.revalidateItem = function (self) {
