 ...
 * [returns] - bool
 *--------------------------------------------------------------*/
NXQUOTE.log = function (payload1, payload2) {
 ...
 *--------------------------------------------------------------*/
NXQUOTE.toggleTaxDiscountButtons = function (value = 0) {

    var bill_final_amount = Number($("#bill_final_amount").val());
    var bill_subtotal = Number($("#bill_subtotal").val());

    //log
    NXQUOTE.log('[billing] toggleTaxDiscountButtons() - based on billing value [bill_final_amount]: (' + bill_final_amount + ')');
...
NXQUOTE.DOM.domState = function () {

    NXQUOTE.log('[billing] state() - setting billing doma state- [payload]', NXQUOTE.DATA.QUOTE);

    //toggle discount and tax buttons

    NXQUOTE.toggleTaxDiscountButtons();
 ...
 *--------------------------------------------------------------*/
NXQUOTE.toggleTaxDom = function (tax_type = '') {



    NXQUOTE.log('[billing] initialiseTaxPopover() - toggling tax popover - [tax_type]: ' + tax_type);
 ...
 *--------------------------------------------------------------*/
NXQUOTE.toggleDiscountDom = function (bill_discount_type = '') {

    //get current values
    var bill_discount_value = '';

    //log
    NXQUOTE.log('[billing] toggleDiscountDom() setting popover state [current_discount_type]: (' + bill_discount_type + ')');
 ...
 *--------------------------------------------------------------*/
NXQUOTE.toggleAdjustmentDom = function () {

    NXQUOTE.log('[billing] toggleAdjustmentDom() setting adjustment popover state');
...
NXQUOTE.DOM.updateAmountDue = function (bill_final_amount = 0) {

    NXQUOTE.log("[billing] NXQUOTE.DOM.updateAmountDue() - updating amount due label [bill_final_amount]: (" + bill_final_amount + ")");
 ...
 *--------------------------------------------------------------*/
NXQUOTE.DOM.itemNewLine = function (data = {}) {

    NXQUOTE.log("[billing] NXQUOTE.DOM.itemNewLine() - cloning time line - [payload]");
    NXQUOTE.log(data);
    ...
    if (item_linked_type != '') {
        ...
        if ($("input[data-duplicate-check='" + check + "']").length > 0) {
            NXQUOTE.log("[billing] NXQUOTE.DOM.itemNewLine() - the item being added is a duplicate. Will skip (" + check + ")");
            //note this duplcate error
            NXQUOTE.DATA.expense_duplicate_count++;
            return;
        }
    }
 ...
 * -add new blank line item
 *--------------------------------------------------------------*/
NXQUOTE.DOM.timeNewLine = function (data = {}) {

    NXQUOTE.log("[billing] NXQUOTE.DOM.timeNewLine() - cloning time line - [payload]");
    NXQUOTE.log(data);

    //get data is any was provided
    var item_description = (data.item_description != null) ? data.item_description : '';
    var item_unit = (data.item_unit != null) ? data.item_unit : NXLANG.quote_time_unit;
 ...
 * ------------------------------------------------------------------------------------------------------------------*/
NXQUOTE.CALC.recalculateLines = function () {

    NXQUOTE.log("[billing] recalculateLines() - validating and recalculating each line item");

    //(1) ------------------------------ find each line item --------------------------------
    $("#billing-items-container").find(".billing-line-item").each(function () {

        NXQUOTE.log("[billing] recalculateLines() - found a line item. Now validating and calculating it");
        ...
        NXQUOTE.log("[billing] recalculateLines() - [quantity]: (" + quantity + ") - [rate]: (" + rate + ") - [linetax]: (" + line_tax + ") - [type]: (" + type + ")");
        ...
        if (type == 'plain') {
            ...
            if (quantity > 0 && rate > 0) {
                ...
                tax.val(linetax);
                //increase bill total
                NXQUOTE.DATA.calc_total += linetotal;
                NXQUOTE.log("[billing] reclaculateBill() - line item is valid. [line item total]: " + linetotal);
            } else {
                NXQUOTE.log("[billing] reclaculateBill() - line item is invalid and is skipped");
                total.val('');
            }
        }
 ...
 * -delete line item
 *--------------------------------------------------------------*/
NXQUOTE.DOM.deleteLine = function (self) {

    NXQUOTE.log("[billing] NXQUOTE.DOM.deleteLine() - deleteing line item");
 ...
 *--------------------------------------------------------------*/
NXQUOTE.DOM.addSelectedProductItems = function (self) {

    NXQUOTE.log("[billing] NXQUOTE.DOM.addSelectedProductItems() - adding items selected in add items modal");
 ...
 *--------------------------------------------------------------*/
NXQUOTE.DOM.addSelectedExpense = function (self) {

    NXQUOTE.log("[billing] NXQUOTE.DOM.addSelectedExpense() - adding expenses selected in add expenses modal");

    //count
    var count_selected = 0;

    //duplicates checker
    NXQUOTE.DATA.expense_duplicate_count = 0;
    ...
    if (NXQUOTE.DATA.expense_duplicate_count) {
        NX.notification({
            type: 'error',
            message: NXLANG.selected_expense_is_already_on_quote
        });
    }
 ...
 *--------------------------------------------------------------*/
NXQUOTE.DOM.addSelectedTimebilling = function (self) {

    NXQUOTE.log("[billing] NXQUOTE.DOM.addSelectedTimebilling() - adding hours selected in add time billing modal");
 ...
 *--------------------------------------------------------------*/
NXQUOTE.updateTax = function () {

    //get tax type from popover form
    var tax_type = $("#billing-tax-type").val();


    NXQUOTE.log("[billing] updateTaxType() - updating tax type [type]: (" + tax_type + ")");
    ...
    //recalculate bill
    NXQUOTE.CALC.reclaculateBill(self);
}
 ...
 * -
 *--------------------------------------------------------------*/
NXQUOTE.updateAdjustment = function () {

    NXQUOTE.log("[billing] updateAdjustment() - updating bill adjustment amount");
 ...
 * -
 *--------------------------------------------------------------*/
NXQUOTE.removeAdjustment = function () {

    //log
    NXQUOTE.log("[billing] removeAdjustment() - updating bill adjustment amount to zero");
 ...
 *--------------------------------------------------------------*/
NXQUOTE.updateDiscount = function () {

    NXQUOTE.log("[billing] updateDiscountType() - updating discount type");
    ...
    if (discount_type == 'percentage') {
        NXQUOTE.log("[billing] updateDiscountType() - [percentage] ", Number($("#js_bill_discount_percentage").val()));
    ...
    if (discount_type == 'fixed') {
        NXQUOTE.log("[billing] updateDiscountType() - [fixed] ", Number($("#js_bill_discount_amount").val()));
 ...
 *--------------------------------------------------------------*/
NXQUOTE.CALC.reclaculateBill = function (self) {

    NXQUOTE.log("[billing] reclaculateBill() - recalculating bill");
    ...
    //recalculate lines
    NXQUOTE.CALC.recalculateLines();



    //(1) ----------------------- SUM UP LINE ITEMS -------------------------------------
    NXQUOTE.log("[billing] reclaculateBill() - summing up all line items - started");
    $("#billing-items-container").find(".billing-line-item").each(function () {
        //each line
        var lineitem = $(this);
        ...
        if (typeof linetotal == 'number') {
            NXQUOTE.log("[billing] reclaculateBill() - valid line item found [total]: (" + linetotal + ")");
            bill_subtotal += linetotal;
        }
    });

    //(2) ----------------- UPDATE SUBTOTAL (SUM BEFORE ADJUSTMENTS)--------------------------------
    NXQUOTE.log("[billing] reclaculateBill() - updating subtotal [subtotal]: (" + bill_subtotal + ") ");
    ...
    NXQUOTE.log("[billing] reclaculateBill() - calculating discounts [bill_discount_type]: (" + bill_discount_type + ") [bill_discount_percentage]: (" + bill_discount_percentage + ") [bill_discount_amount] (" + bill_discount_amount + ")");
    ...
    if (bill_discount_type == 'percentage') {
        ...
        //log
        NXQUOTE.log("[billing] reclaculateBill() - discount is percentage based. [bill_discount_amount]: (" + bill_discount_amount + ")");
    }
    ...
    if (bill_discount_type == 'fixed') {
        //log
        NXQUOTE.log("[billing] reclaculateBill() - discount is fixed. [bill_discount_amount]: (" + bill_discount_amount + ")");
    }
    ...
    if (bill_discount_amount > 0) {
        ...
        //log
        NXQUOTE.log("[billing] reclaculateBill() - there is a discount - setting DOM  and [bill_amount_before_tax]: (" + bill_amount_before_tax + ")");
        ...
        //log
        NXQUOTE.log("[billing] reclaculateBill() - there is no discount - [bill_amount_before_tax]: (" + bill_amount_before_tax + ")");
    }
    ...
    if ($("#bill_tax_type").val() == 'summary') {
        //log
        NXQUOTE.log("[billing] reclaculateBill() - calculating summary taxes");
        ...
        if (bill_tax_total_percentage > 0) {
            //log
            NXQUOTE.log("[billing] reclaculateBill() - this bill has [summary] based tax [tax_percentage]: (" + bill_tax_total_percentage + "%)");
            //tax calculation
            ...
            //log
            NXQUOTE.log("[billing] reclaculateBill() - the total tax on this bill is [total_tax_amount]: (" + bill_tax_total_amount + ")");
        } else {
            NXQUOTE.log("[billing] reclaculateBill() - this bill does not have any applicable tax");
    ...
    NXQUOTE.log("[billing] reclaculateBill() - total amount before adjustment. [bill_final_amount]: (" + bill_final_amount + ")");

    //adjustment
    NXQUOTE.log("[billing] reclaculateBill() - accounting for the adjustment. [bill_adjustment_amount]: (" + bill_adjustment_amount + ")");
    ...
    NXQUOTE.DOM.updateAmountDue(bill_final_amount);

    NXQUOTE.log("[billing] reclaculateBill() - bill claculation finished [final amount]: (" + bill_final_amount + ")");
 ...
 * [SAVE QUOTE BUTTON]
 *--------------------------------------------------------------*/
NXQUOTE.CALC.saveBill = function (self) {

    NXQUOTE.log("[billing] saveBill() - started");

    if (NXQUOTE.CALC.validateLines()) {
 ...
 * [SAVE QUOTE BUTTON]
 *--------------------------------------------------------------*/
NXQUOTE.CALC.validateLines = function (self) {

    //log
    NXQUOTE.log("[billing] validateLines() - started");
    ...
    //log
    NXQUOTE.log("[billing] validateLines() - (" + count_bill_error + ") error found");
 ...
 *--------------------------------------------------------------*/
NXQUOTE.DOM.revalidateItem = function (self) {
