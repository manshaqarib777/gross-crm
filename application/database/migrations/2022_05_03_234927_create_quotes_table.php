...
class CreateQuotesTable extends Migration {
	...
	public function up()
	{
		...
		{
			$table->integer('bill_quoteid', true);
			$table->dateTime('bill_created')->nullable();
			$table->dateTime('bill_updated')->nullable();
			$table->date('bill_date_sent_to_customer')->nullable()->comment('the date an quote was published or lasts emailed to the customer');
			$table->integer('bill_clientid')->index('quote_clientid');
			$table->integer('bill_projectid')->nullable()->index('quote_projectid')->comment('optional');
			$table->integer('bill_subscriptionid')->nullable()->comment('optional');
			$table->integer('bill_creatorid')->index('quote_creatorid');
			$table->integer('bill_categoryid')->default(4)->index('quote_categoryid');
			...
			$table->string('bill_status', 50)->default('draft')->index('quote_status')->comment('draft | due | overdue | paid | part_paid');
			$table->string('bill_recurring', 50)->nullable()->default('no')->index('quote_recurring')->comment('yes|no');
			...
			$table->integer('bill_recurring_parent_id')->nullable()->comment('if it was generated from a recurring quote, the id of parent quote');
			$table->string('bill_overdue_reminder_sent', 5)->nullable()->default('no')->comment('yes | no');
			$table->string('bill_quote_type', 30)->nullable()->default('onetime')->index('bill_quote_type')->comment('onetime | subscription');
			$table->string('bill_type', 20)->nullable()->default('quote')->index('bill_type')->comment('quote|estimate');
			$table->string('bill_visibility', 20)->nullable()->default('visible')->comment('visible|hidden (used to prevent quotes that are still being cloned from showing in quotes list)');
			$table->string('bill_cron_status', 20)->nullable()->default('none')->comment('none|processing|completed|error  (used to prevent collisions when recurring quoted)');
