...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the bol-payments
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/bol-payments/components/table/ajax';
                $dom_container = '#bol-payments-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/bol-payments/components/table/table';
                $dom_container = '#bol-payments-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //showing from bol page (inside a modal)
            case 'bol-bol-payments-modal':
                $template = 'pages/bol-payments/components/table/table';
            ...
            default:
                $template = 'pages/bol-payments/tabswrapper';
            ...
            if ($bol-payments->currentPage() < $bol-payments->lastPage()) {
                $url = loadMoreButtonUrl($bol-payments->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'bol-payments', 'stats', 'projects'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($bol-payments->currentPage() + 1, request('source'));
            $page['loading_target'] = 'bol-payments-td-container';
            $page['visibility_show_load_more'] = ($bol-payments->currentPage() < $bol-payments->lastPage()) ? true : false;
            return view('pages/bol-payments/wrapper', compact('page', 'bol-payments', 'stats', 'projects'))->render();
        }
