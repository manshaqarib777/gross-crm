...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the quote-payments
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/quote-payments/components/table/ajax';
                $dom_container = '#quote-payments-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/quote-payments/components/table/table';
                $dom_container = '#quote-payments-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //showing from quote page (inside a modal)
            case 'quote-quote-payments-modal':
                $template = 'pages/quote-payments/components/table/table';
            ...
            default:
                $template = 'pages/quote-payments/tabswrapper';
            ...
            if ($quote-payments->currentPage() < $quote-payments->lastPage()) {
                $url = loadMoreButtonUrl($quote-payments->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'quote-payments', 'stats', 'projects'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($quote-payments->currentPage() + 1, request('source'));
            $page['loading_target'] = 'quote-payments-td-container';
            $page['visibility_show_load_more'] = ($quote-payments->currentPage() < $quote-payments->lastPage()) ? true : false;
            return view('pages/quote-payments/wrapper', compact('page', 'quote-payments', 'stats', 'projects'))->render();
        }
