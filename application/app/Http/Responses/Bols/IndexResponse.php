...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the bols
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/bols/components/table/ajax';
                $dom_container = '#bols-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/bols/components/table/table';
                $dom_container = '#bols-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/bols/tabswrapper';
            ...
            if ($bols->currentPage() < $bols->lastPage()) {
                $url = loadMoreButtonUrl($bols->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'bols', 'stats', 'categories', 'tags', 'projects'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($bols->currentPage() + 1, request('source'));
            $page['loading_target'] = 'temp-td-container';
            $page['visibility_show_load_more'] = ($bols->currentPage() < $bols->lastPage()) ? true : false;
            return view('pages/bols/wrapper', compact('page', 'bols', 'stats', 'categories', 'tags', 'projects'))->render();
        }
