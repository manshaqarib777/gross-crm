...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the quotes
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/quotes/components/table/ajax';
                $dom_container = '#quotes-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/quotes/components/table/table';
                $dom_container = '#quotes-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/quotes/tabswrapper';
            ...
            if ($quotes->currentPage() < $quotes->lastPage()) {
                $url = loadMoreButtonUrl($quotes->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'quotes', 'stats', 'categories', 'tags', 'projects'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.quotes')),
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($quotes->currentPage() + 1, request('source'));
            $page['loading_target'] = 'temp-td-container';
            $page['visibility_show_load_more'] = ($quotes->currentPage() < $quotes->lastPage()) ? true : false;
            return view('pages/quotes/wrapper', compact('page', 'quotes', 'stats', 'categories', 'tags', 'projects'))->render();
        }
