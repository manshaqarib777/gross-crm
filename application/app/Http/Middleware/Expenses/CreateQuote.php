...
class CreateQuote {
    ...
    public function handle($request, Closure $next) {
        ...
        //check if not already quoted
        if ($expense->expense_billing_status == 'quoted') {
            abort(409, __('lang.expense_has_already_been_quoted'));
        }
    ...
    private function fronteEnd($expense) {
        ...
        //defaults
        config([
            'visibility.quote_modal_expenses_payload' => true,
        ]);
        ...
        if (!is_numeric($expense->expense_projectid) && !is_numeric($expense->expense_clientid)) {
            ...
            config([
                'visibility.quote_modal_client_project_fields' => true,
