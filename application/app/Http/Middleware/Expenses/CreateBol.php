...
class CreateBol {
    ...
    public function handle($request, Closure $next) {
        ...
        //check if not already bold
        if ($expense->expense_billing_status == 'bold') {
            abort(409, __('lang.expense_has_already_been_bold'));
        }
    ...
    private function fronteEnd($expense) {
        ...
        //defaults
        config([
            'visibility.bol_modal_expenses_payload' => true,
        ]);
        ...
        if (!is_numeric($expense->expense_projectid) && !is_numeric($expense->expense_clientid)) {
            ...
            config([
                'visibility.bol_modal_client_project_fields' => true,
