...
/** --------------------------------------------------------------------------------
 * This middleware class handles [index] precheck processes for bols
 *
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class Index {
    ...
    public function handle($request, Closure $next) {
        ...
        if (request()->filled('bolresource_type') && request()->filled('bolresource_id')) {
            ...
            if (request('bolresource_type') == 'project') {
                request()->merge([
                    'filter_bill_projectid' => request('bolresource_id'),
                ]);
            }
            //client bols
            if (request('bolresource_type') == 'client') {
                request()->merge([
                    'filter_bill_clientid' => request('bolresource_id'),
                ]);
            }
        }
        ...
        if (auth()->user()->is_client) {
            if (auth()->user()->is_client_owner) {
                //exclude draft bols & sanity client
                request()->merge([
                    'filter_bol_exclude_status' => 'draft',
        ...
        //permission denied
        Log::error("permission denied", ['process' => '[permissions][bols][index]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
        abort(403);
    }
    ...
    private function fronteEnd() {
         ...
         * */
        if (request('bolresource_type') != '' || is_numeric(request('bolresource_id'))) {
            request()->merge([
                'resource_query' => 'ref=list&bolresource_type=' . request('bolresource_type') . '&bolresource_id=' . request('bolresource_id'),
            ]);
        } else {
            request()->merge([
        ...
        config([
            'visibility.bols_col_client' => true,
            'visibility.bols_col_project' => true,
            'visibility.bols_col_payments' => true,
        ...
        if (auth()->user()->role->role_bols >= 1) {
            ...
            if (auth()->user()->is_client) {
                config([
                    //visibility
                    'visibility.list_page_actions_search' => true,
                    'visibility.bols_col_client' => false,
                ]);
            }
        }
        ...
        if (auth()->user()->role->role_bols >= 2) {
            config([
                ...
                'visibility.bols_col_checkboxes' => true,
            ]);
        }
        ...
        if (request('bolresource_type') == 'project') {
            config([
                //visibility
                'visibility.bols_col_client' => false,
                'visibility.bols_col_project' => false,
        ...
        if (request('bolresource_type') == 'client') {
            config([
                //visibility
                'visibility.bols_col_client' => false,
                'visibility.bols_col_payments' => false,
                'visibility.filter_panel_client_project' => false,
