...
class BolPaymentRepository {
    ...
    public function search($id = '', $data = array()) {
        ...
        $payments->leftJoin('bols', 'bols.bill_bolid', '=', 'payments.payment_bolid');
        ...
        if ($data['apply_filters']) {
            ...
            if (request()->filled('filter_payment_bolid')) {
                $payments->where('payment_bolid', request('filter_payment_bolid'));
            }
            ...
            if (request()->filled('search_query') || request()->filled('query')) {
                $payments->where(function ($query) {
                    ...
                    $query->Where('payment_bolid', '=', $bill_bolid);
    ...
    public function create() {
        ...
        $payment->payment_bolid = request('payment_bolid');
        ...
        if ($payment->save()) {
            ...
            Log::error("record could not be saved - database error", ['process' => '[BolPaymentRepository]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
    }
