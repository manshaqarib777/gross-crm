        ...
        <table>
            <tr>
                ...
                <td class="x-payments"> <a href="javascript:void(0)" class="edit-add-modal-button js-ajax-ux-request reset-target-modal-form"
                        ...
                        data-url="{{ urlResource('/payments?action=bol-payments-modal&paymentresource_type=bol&paymentresource_id='.$bill->bill_bolid) }}"
            ...
            <tr>
                ...
                <td class="x-balance-due"> <span class="x-due-amount-label">
                        @if($bill->bol_balance > 0)
                        <label class="label label-rounded label-danger"
                            id="billing-details-amount-due">{!! runtimeMoneyFormatPDF($bill->bol_balance) !!}</label>
                        @else
                    ...
                    <!--pdf-->
                    <span class="x-due-amount-plain hidden">{!! runtimeMoneyFormatPDF($bill->bol_balance) !!}</span>
                </td>
            </tr>
        </table>
    </div>